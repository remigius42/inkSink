...
- [ ] 1.1 Fix `ansible.cfg`: add `[facts_caching]` section (`fact_caching =
  jsonfile`, `fact_caching_connection = .ansible/facts_cache`,
  `fact_caching_timeout = 86400`); add missing `[defaults]` keys: `nocows = 1`,
  `retry_files_enabled = False`, `display_skipped_hosts = no`,
  `allow_world_readable_tmpfiles = True`
- [ ] 1.2 Add SPDX header (`# SPDX-License-Identifier: MIT`) to
  `ansible/inventory/hosts.yml`
- [ ] 1.3 Add SPDX header to `ansible/group_vars/all/vars.yml`; add
  `fail2ban_bantime`, `fail2ban_findtime`, `fail2ban_maxretry` defaults
- [ ] 1.4 Add SPDX header to `ansible/group_vars/all/vault.yml.example`

## 2. `base` Role

- [ ] 2.1 Create `roles/base/defaults/main.yml` and `meta/main.yml` (SPDX
  headers; meta declares `community.general` and `ansible.posix` dependencies)
- [ ] 2.2 Fix locale tasks: add `de_CH.UTF-8` locale (currently only
  `en_US.UTF-8`); add `community.general.timezone` task using
  `base_locale_timezone`
- [ ] 2.3 Fix SSH hardening: add missing `PubkeyAuthentication yes` lineinfile
  task
- [ ] 2.4 Fix apt package list: remove `python3-pillow`, `python3-requests`,
...
- [ ] 2.5 Implement fail2ban tasks: deploy SSH jail drop-in to
  `/etc/fail2ban/jail.d/sshd.conf` using
  `fail2ban_bantime`/`fail2ban_findtime`/`fail2ban_maxretry` vars; enable and
  start service; add `Restart fail2ban` handler
- [ ] 2.6 Move Waveshare vendor sync here from `inksink` role:
...
- [ ] 2.7 Add SPDX header to `roles/base/tasks/main.yml`
- [ ] 2.8 Add SPDX header to `roles/base/handlers/main.yml`; replace
  `ansible.builtin.service` with `ansible.builtin.systemd` (add `daemon_reload:
  true`, `become: true`); add `Restart fail2ban` handler
- [ ] 2.9 Add `become: true` to every privileged task in
  `roles/base/tasks/main.yml`; remove `become: true` from playbook level in all
  three playbooks

## 3. `inksink` Role

- [ ] 3.1 Create `roles/inksink/defaults/main.yml` and `meta/main.yml` (SPDX
  headers); add `inksink_venv_dir: /opt/inksink-venv` to
  `ansible/group_vars/all/vars.yml`
- [ ] 3.2 Fix directory task: add `/var/lib/inksink/` (pi:pi 0750) to the loop;
  remove `{{ inksink_vendor_dir }}` (now managed by `base`)
- [ ] 3.3 Remove Waveshare vendor sync tasks from `roles/inksink/tasks/main.yml`
  (moved to `base`)
- [ ] 3.4 Fix config template deploy: change mode from `"0600"` to `"0640"` (per
  `inksink-service` spec)
- [ ] 3.5 Convert `roles/inksink/files/inksink.service` from static file to
...
- [ ] 3.6 Add SPDX header to `roles/inksink/tasks/main.yml`; add `become: true`
  to all privileged tasks
- [ ] 3.7 Add SPDX header to `roles/inksink/handlers/main.yml`; replace
  `ansible.builtin.service` with `ansible.builtin.systemd` (`become: true`)
- [ ] 3.8 Create app virtualenv: add `ansible.builtin.command` task to run
  `python3 -m venv {{ inksink_venv_dir }}` (with `creates:` guard so it is
  idempotent); set ownership to `pi:pi`
- [ ] 3.9 Install Python dependencies into venv: `ansible.builtin.pip` task with
...
- [ ] 3.10 Update `roles/inksink/templates/inksink.j2` wrapper to use `{{
  inksink_venv_dir }}/bin/python3` instead of bare `python3`

## 4. Playbooks

- [ ] 4.1 Add SPDX header to `playbooks/setup.yml`, `deploy.yml`, `verify.yml`
- [ ] 4.2 Remove `become: true` from play level in all three playbooks
...
- [ ] 5.1 Create `ansible/roles/base/README.md` and
  `ansible/roles/inksink/README.md` with variable reference tables
- [ ] 5.2 Update `docs/setup.md` Step 1 to make explicit that pasting the SSH
  ...
  authentication)
- [ ] 5.3 Update `README.md` Build Guide section to link to `docs/setup.md`
- [ ] 5.4 Run pre-commit hooks (`pre-commit run --all-files`) and fix any issues
- [ ] 5.5 `pyproject.toml` `[tool.pyright]`: add `exclude = ["vendor"]` so
  vendored Waveshare Python files are not type-checked
- [ ] 5.6 `.cspell.yaml` `ignorePaths`: add `vendor/**/*.py` and
