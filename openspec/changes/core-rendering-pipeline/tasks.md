...
- [ ] 1.1 Define `Orientation(str, enum.Enum)` with `PORTRAIT = "portrait"`
...
- [ ] 1.2 Delete `_HTML_TEMPLATE` and the fragment-wrapping logic; `render()`
...
- [ ] 3.3 Create `core/layouts/default.html.j2` — status bar row (time, WiFi,
  ...
  `calc(100vh - 104px)`
- [ ] 3.4 Implement `fill_fullscreen(content: str) -> str` in `core/layout.py`:
  load and render `fullscreen.html.j2` with `content`
- [ ] 3.5 Implement `fill_default(content: str, buttons: list[str]) -> str`:
...
- [ ] 3.6 Write unit tests: `fill_fullscreen` returns HTML containing content;
...
- [ ] 4.1 Replace `docs/adr/0008-portrait-landscape-orientation.md` with the
...
- [ ] 4.2 Define `_PANEL_W: int = 800` and `_PANEL_H: int = 480` in
...
- [ ] 4.3 Run full test suite; fix any callers broken by renderer signature change
- [ ] 4.4 Ensure all changed and new production code has type hints on function
  signatures and module-level variables
- [ ] 4.5 Ensure all new modules have a module-level docstring; add function
  docstrings where the purpose is not obvious from the signature
- [ ] 4.6 Ensure all changed and new production code is covered by unit tests;
  no new logic without a corresponding test
- [ ] 4.7 Run pre-commit on all modified files (`pre-commit run --files <files>`)
  and fix all violations before marking tasks complete

## 5. Button input — generic positional IDs

- [ ] 5.1 Add the two truly new GPIO entries to `_DEFAULT_PIN_MAP`: `btn_5`
  (GPIO 17) and `btn_8` (GPIO 27); confirm both GPIO assignments against
  physical HAT wiring before finalizing
- [ ] 5.2 Rename all existing Anki-specific keys in `_DEFAULT_PIN_MAP`:
  `power`→`btn_1`, `show_answer`→`btn_2`, `again`→`btn_3`, `hard`→`btn_4`,
  `good`→`btn_6`, `easy`→`btn_7`
- [ ] 5.3 Remove any `setup()` logic that treated the former `power` entry as
  a special case (e.g. shutdown callbacks on GPIO 4); GPIO 4 is now a regular
  softkey (`btn_1`)
- [ ] 5.4 Update existing unit tests: replace `"show_answer"`, `"again"`,
  `"hard"`, `"good"`, `"easy"` assertions with the new `btn_*` IDs
- [ ] 5.5 Search codebase for all `wait_for_action()` callers matching on
