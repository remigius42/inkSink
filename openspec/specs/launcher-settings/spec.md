...
The Launcher SETTINGS screen shows the current device configuration read-only.
Reached via `btn_6` in MENU. Displays all `load_settings()` keys as a flat
dot-notation list with credential values masked. Supports vertical scrolling
for configs that exceed the visible content area. No editing — SSH + deploy
covers config changes.
