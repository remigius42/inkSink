...
On-demand hardware state reads for battery level (PiSugar 3 via I2C) and
WiFi status (via `nmcli`). Returns sentinel values when hardware or tools
are unavailable, safe to call on dev machines.
