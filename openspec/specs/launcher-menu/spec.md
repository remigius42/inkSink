...
Apps as labeled buttons and routes user input to App launches, system screens
(Status, Settings, Logs), or sleep. `__main__.py` owns the infinite loop;
`Launcher.run()` is single-pass — it renders MENU, handles one selection, then
returns.
