...
The Launcher LOGS screen shows recent systemd journal entries for the `inksink`
service on demand. Reached via `btn_7` in MENU. Displays the last 100 lines
from `journalctl`, starting at the bottom (most recent visible). Supports the
same scroll mechanism as the SETTINGS screen. Degrades to "unavailable" when
`journalctl` is absent or fails.
