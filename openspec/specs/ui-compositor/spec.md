...
- **WHEN** `set_buttons(["A", "B", "C", "D", "", "F", "G", "H"], states)` is called
  with `""` at index 4 (first slot of row 2 in portrait 4×2 layout)
- **THEN** `ValueError` is raised with a message identifying the offending slot
  index and the rule, e.g. `"slot 4: '' cannot start a row"`
